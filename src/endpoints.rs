...
pub async fn mavlink_post(
    ...
    match json5::from_str::<data::MAVLinkMessage<mavlink::ardupilotmega::MavMessage>>(&json_string)
    {
        Ok(content) => match data.lock().unwrap().send(&content.header, &content.message) {
            Ok(_result) => {
